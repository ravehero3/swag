    ...
    >
      ...
      >
        <div style={{ display: "?"flex"?", justifyContent: "?"space-between"?", alignItems: "?"center"?", marginBottom: "?"16px"?" }}>
          ...
          <button onClick={onClose} style={{ background: "?"none"?", border: "?"none"?", color: "?"#555"?", cursor: "?"pointer"?", fontSize: "?"20px"?", lineHeight: 1 }"}>Close</button>
        </div>
...
function GDriveLinkStatus({ url }: { url: string }) {
  ...
  const icon = state.status === "?"ok"?" ? "?"[OK]"?" : state.status === "?"bad"?" ? "?"[No]"?" : state.status === "?"checking"?" ? "?"..."?" : "?"?"?";
    ...
    <div style={{ marginTop: "?"8px"?", padding: "?"10px"?", background: "?"#0f0f0f"?", border: "?"1px solid #2a2a2a"?", borderRadius: "?"4px"?" }}>
      <div style={{ display: "?"flex"?", gap: "?"12px"?", alignItems: "?"flex-start"?" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {status === "?"ok"?" && (
            <div style={{ fontSize: "?"12px"?", color: "?"#4caf50"?", marginBottom: "?"4px"?" }}>[OK] Image loaded – tak ho uvidí návštěvníci</div>
          )}
          {status === "?"error"?" && (
      ...
      <div style={{ marginTop: "?"8px"?", background: "?"#0d0d0d"?", border: "?"1px solid #222"?", borderRadius: "?"8px"?", padding: "?"10px 12px"?" }}>
        ...
        <div style={{ marginTop: "?"6px"?", display: "?"flex"?", justifyContent: "?"space-between"?", alignItems: "?"center"?" }}>
          <span style={{ fontSize: "?"11px"?", color: isDone ? "?"#4caf50"?" : isProcessing ? "?"#0B99FC"?" : "?"#666"?" }}>
            {isDone ? "?"[OK] Uploaded"?" : isProcessing ? "?"Processing on server..."?" : `Uploading... ${pct}%`}
          </span>
          {!isDone && !isProcessing && (
    ...
    <div>
      ...
      <div style={{ display: "?"flex"?", gap: "?"8px"?", marginBottom: "?"20px"?", flexWrap: "?"wrap"?", alignItems: "?"center"?" }}>
        <button
          ...
          style={{ fontSize: "?"13px"?" }}
        >
          {showForm ? "?"× Zavřít formulář"?" : "?"Add Beat"?"}
        </button>
        ...
        {beats.some((b: any) => !b.is_published) && (
          <button className="?"btn btn-admin"?" onClick={handlePublishAll} style={{ borderColor: "?"#4caf50"?", color: "?"#4caf50"?", fontSize: "?"13px"?" }}>
            Publish skryté ({beats.filter((b: any) => !b.is_published).length})
          </button>
        )}
                ...
                <div style={{ marginBottom: "?"12px"?", display: "?"flex"?", flexDirection: "?"column"?", gap: "?"3px"?" }}>
                  {folderQueue.map(item => (
                    <div key={item.id} style={{
                      ...
                      {item.status === "?"error"?"    && <button onClick={() => setFolderQueue(prev => prev.filter(q => q.id !== item.id))} style={{ background: "?"none"?", border: "?"none"?", color: "?"#555"?", cursor: "?"pointer"?", fontSize: "?"14px"?", padding: "?"0 2px"?", lineHeight: 1, flexShrink: 0 }"}>Close</button>}
                    </div>
                  ))}
                </div>
              )}
...
function KitsTab({ kits, showForm, setShowForm, editing, setEditing, onRefresh }: any) {
  ...
  useEffect(() => {
    if (editing) {
      ...
      // "?"[OK] Uploaded – bezpečné pokračovat"?" bar doesn't appear before the user
  ...
  const uploadFile = async (file: File, type: string) => {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Upload failed';
      setUploadError(prev => ({ ...prev, [type]: errorMsg }));
      // Reset the progress bar so the user doesn't see a green "?"[OK] Uploaded"?" banner
      ...
      <div style={{ marginTop: "?"8px"?" }}>
        <div style={{ display: "?"flex"?", justifyContent: "?"space-between"?", alignItems: "?"center"?", marginBottom: "?"6px"?" }}>
          {isUploading ? (
            ...
            <>
              <span style={{ fontSize: "?"12px"?", color: "?"#4caf50"?" }}>[OK] Uploaded – bezpečné pokračovat</span>
              ...
              <div style={{ display: "?"flex"?", flexWrap: "?"wrap"?", gap: "?"4px"?", marginTop: "?"8px"?" }}>
                {form.tags.map((tag, i) => (
                  <span key={i} style={{ padding: "?"4px 8px"?", border: "?"1px solid #fff"?", fontSize: "?"12px"?" }}>
                    {tag} <button type="?"button"?" onClick={() => setForm({ ...form, tags: form.tags.filter((_, j) => j !== i) })} style={{ background: "?"none"?", border: "?"none"?", color: "?"#fff"?", cursor: "?"pointer"?" }"}>Close</button>
                  </span>
                ))}
              </div>
            </div>
      ...
      {previewHtml && (
        ...
        >
          ...
          >
            ...
            <button onClick={() => setPreviewHtml(null)} style={{ background: "?"rgba(255,255,255,0.1)"?", border: "?"1px solid rgba(255,255,255,0.15)"?", borderRadius: "?"4px"?", fontSize: "?"13px"?", cursor: "?"pointer"?", color: "?"#ccc"?", padding: "?"4px 12px"?" }}>Close</button>
          </div>
          {/* A4 paper */}
          <div
            onClick={(e) => e.stopPropagation()}
            ...
            <div key={license.id} style={{ border: "?"1px solid #1f1f1f"?", borderRadius: "?"4px"?", overflow: "?"hidden"?" }}>
              ...
              >
                ...
                <div style={{ display: "?"flex"?", gap: "?"12px"?", alignItems: "?"center"?" }}>
                  ...
                  <span style={{ fontSize: "?"11px"?", color: license.contract_template ? "?"#4caf50"?" : "?"#ff6b6b"?" }}>{license.contract_template ? "?"[OK] Contract"?" : "?"[No] No Contract"?"}</span>
    ...
    <div style={{ padding: "?"20px 0"?" }} data-testid="?"tab-slevy"?">
      ...
      <form onSubmit={saveSettings} style={containerStyle}>
        ...
        <div style={{ display: "?"flex"?", justifyContent: "?"flex-end"?", marginTop: "?"24px"?" }}>
          ...
          >
            {saving ? "?"Ukládám..."?" : saved ? "?"Saved!"?" : "?"Uložit nastavení"?"}
          </button>
        </div>
      </form>
    </div>
        ...
        {ran && !loading && diag && (
          <div style={{ padding: "?"16px"?" }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: "?"16px"?", flexShrink: 0 }}>{diag.tokenOk ? "?"[OK]"?" : "?"[No]"?"}</span>
              <div>
            ...
            {diag.tokenOk && (
              <div style={{
                ...
                <span style={{ fontSize: "?"16px"?", flexShrink: 0 }}>{diag.paymentTestOk ? "?"[OK]"?" : "?"[No]"?"}</span>
      ...
      {!loading && !error && (
        <>
          <div style={{
            ...
            <span style={{ fontSize: "?"20px"?" }}>{allOk ? "?"[OK]"?" : "?"⚠"?"}</span>
