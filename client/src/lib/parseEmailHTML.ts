  ...
  try {
    ...
    if (headings.length > 0) {
      ...
      if (heading) {
        ...
        console.log("[Parser] Extracted heading block:", headingBlock);
    ...
    paragraphs.forEach((p) => {
      ...
      if (text.length > 20 && !text.includes("©") && !text.includes("@")) {
        ...
        console.log("[Parser] Extracted paragraph block:", paragraphBlock);
    ...
    infoBoxDivs.forEach((box) => {
      ...
      if (pElements.length >= 2) {
        ...
        if (title && content.length > 10) {
          ...
          console.log("[Parser] Extracted info_box block:", infoBoxBlock);
          blocks.push(infoBoxBlock);
        }
      }
    });

    console.log("[Parser] Total blocks extracted:", blocks.length, blocks);
  } catch (error) {
