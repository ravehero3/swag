import { useEffect, useState } from "react";

interface Artist {
  name: string;
  id: string;
  imageUrl?: string;
}

const artists: Artist[] = [
  { name: "Dollar Prync", id: "dollar-prync", imageUrl: "/uploads/artwork/dollar_prync.png" },
  { name: "Smack", id: "smack", imageUrl: "/uploads/artwork/smack.png" },
  { name: "Nik Tendo", id: "nik-tendo", imageUrl: "/uploads/artwork/nik_tendo.png" },
  { name: "Calin", id: "calin", imageUrl: "/uploads/artwork/calin.png" },
  { name: "Hugo Toxxx", id: "hugo-toxxx", imageUrl: "/uploads/artwork/hugo_toxxx.png" },
  { name: "Viktor Sheen", id: "viktor-sheen", imageUrl: "/uploads/artwork/viktor_sheen.png" },
  { name: "Yzomandias", id: "yzomandias", imageUrl: "/uploads/artwork/yzomandias.png" },
  { name: "Ektor", id: "ektor", imageUrl: "/uploads/artwork/ektor.png" },
  { name: "Ca$hanova Bulhar", id: "cashanova-bulhar", imageUrl: "/uploads/artwork/cashanova_bulhar.png" },
  { name: "Hasan", id: "hasan", imageUrl: "/uploads/artwork/hasan.png" },
  { name: "Luca Brassi10x", id: "luca-brassi10x", imageUrl: "/uploads/artwork/luca_brassi10x.png" },
  { name: "Hard Rico", id: "hard-rico", imageUrl: "/uploads/artwork/hard_rico.png" },
  { name: "Pil C", id: "pil-c", imageUrl: "/uploads/artwork/pil_c.png" },
];

const MobileCarousel = () => {
  const [offset, setOffset] = useState(0);

  // Duplicate artists 3 times to ensure seamless scrolling
  const displayArtists = [
    ...artists,
    ...artists,
    ...artists,
  ];

  useEffect(() => {
    const imageWidth = 70;
    const gap = 8;
    const singleSetWidth = imageWidth * artists.length + gap * artists.length;

    const intervalId = setInterval(() => {
      setOffset((prev) => {
        const newOffset = prev - 1;
        if (newOffset <= -singleSetWidth) {
          return 0;
        }
        return newOffset;
      });
    }, 16);

    return () => clearInterval(intervalId);
  }, []);

  return (
    <div
      className="mobile-carousel-root"
      style={{
        width: "100vw",
        marginLeft: "calc(-50vw + 50%)",
        minHeight: "140px",
        background: "transparent",
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        justifyContent: "center",
        overflow: "hidden",
        position: "relative",
        marginBottom: "64px",
        paddingBottom: "0px",
        padding: "20px 20px",
        zIndex: 100,
      }}
    >
      <style>{`
        @media (max-width: 768px) {
          .mobile-carousel-root {
            position: relative !important;
            z-index: 10000 !important;
          }
          .carousel-root-container-mobile {
            display: none !important;
          }
        }
        @media (min-width: 769px) {
          .mobile-carousel-root {
            display: none !important;
          }
        }
      `}</style>
      <div className="carousel-root-container-mobile" style={{ textAlign: "center", marginBottom: "8px", paddingTop: "0px", position: "relative", top: "-200px", zIndex: 99999, overflow: "visible", background: "transparent", width: "100vw", marginLeft: "calc(-50vw + 50%)" }}>
        <style>{`
          @media (max-width: 768px) {
            .carousel-root-container-mobile {
              display: none !important;
            }
          }
        `}</style>
        <p style={{ fontSize: "12px", color: "#555", margin: 0, fontFamily: "Work Sans, sans-serif" }}>
          VOODOO808 dělal beaty pro
        </p>
      </div>

      {/* Carousel Container */}
      <div
        className="carousel-container-mobile"
        style={{
          width: "100vw",
          marginLeft: "calc(-50vw + 50%)",
          marginRight: "calc(-50vw + 50%)",
          overflow: "hidden",
          position: "relative",
          flex: 1,
          display: "flex",
          alignItems: "center",
        }}
      >
        <div
          className="mobile-carousel-track"
          style={{
            display: "flex",
            gap: "8px",
            transform: `translateX(${offset}px)`,
            transition: "transform 0.05s linear",
            paddingLeft: "0px",
          }}
        >
          {displayArtists.map((artist, index) => (
            <div
              key={`${artist.id}-${index}`}
              className="mobile-carousel-item"
              style={{
                display: "flex",
                flexDirection: "column",
                alignItems: "center",
                gap: "6px",
                flexShrink: 0,
                width: "90px",
              }}
            >
              {/* Artist Image Circle */}
              <div
                className="mobile-artist-image-circle"
                style={{
                  width: "70px",
                  height: "70px",
                  borderRadius: "50%",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  overflow: "hidden",
                  flexShrink: 0,
                  backgroundColor: "#000",
                }}
              >
                {artist.imageUrl ? (
                  <img
                    src={artist.imageUrl}
                    alt={artist.name}
                    style={{
                      width: "100%",
                      height: "100%",
                      objectFit: "cover",
                      objectPosition: "center",
                    }}
                  />
                ) : (
                  <div
                    style={{
                      width: "100%",
                      height: "100%",
                      background: `linear-gradient(135deg, hsl(${(artist.id.charCodeAt(0) * 7) % 360}, 60%, 45%), hsl(${(artist.id.charCodeAt(0) * 7 + 60) % 360}, 60%, 50%))`,
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      color: "#999",
                      fontSize: "24px",
                      fontWeight: "bold",
                      fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif',
                    }}
                  >
                    {artist.name.charAt(0).toUpperCase()}
                  </div>
                )}
              </div>

              {/* Artist Name */}
              <p
                className="mobile-artist-name-text"
                style={{
                  fontSize: "9px",
                  color: "#666",
                  margin: "0",
                  fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif',
                  textAlign: "center",
                  lineHeight: "1.2",
                  maxWidth: "70px",
                  whiteSpace: "normal",
                  wordWrap: "break-word",
                }}
              >
                {artist.name}
              </p>
            </div>
          ))}
        </div>

        {/* Left Gradient Fade */}
        <div
          className="carousel-fade-mobile"
          style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: "100px",
            height: "100%",
            background: "linear-gradient(to right, rgba(0, 0, 0, 1), rgba(0, 0, 0, 0))",
            pointerEvents: "none",
            zIndex: 10,
          }}
        />

        {/* Right Gradient Fade */}
        <div
          className="carousel-fade-mobile"
          style={{
            position: "absolute",
            right: 0,
            top: 0,
            width: "100px",
            height: "100%",
            background: "linear-gradient(to left, rgba(0, 0, 0, 1), rgba(0, 0, 0, 0))",
            pointerEvents: "none",
            zIndex: 10,
          }}
        />
      </div>
    </div>
  );
};

export default MobileCarousel;
